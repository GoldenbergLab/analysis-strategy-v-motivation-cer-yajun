...
my_summ_lm <- function(x, ...) {
    jtools::summ(x, scale=F, transform.response=F, model.info = F , confint = T, digits = 3, ...)
}
...
## estimating main effects by recoding conditions (treatment group)
```{r}
...
## simple effects (treatment subgroup)
```{r}
# Simple effect of method -- recode motivation
## when grp_goal = non-motivated = 0
## i.e. comparing reappraisal only vs. baseline
emoreg_data_treat$grp_goal_simp2 <- NA
emoreg_data_treat$grp_goal_simp2[emoreg_data_treat$grp_goal == "motivated"] <- 1
emoreg_data_treat$grp_goal_simp2[emoreg_data_treat$grp_goal == "non_motivated"] <- 0
lm_simp_tr2 <- lmer(rate ~ grp_goal_simp2*grp_method + size + trial + (1|img) +  (1|groupID/sub_id), emoreg_data_treat)
my_summ_lm(lm_simp_tr2)
#confint(lm_simp_tr2)

## when grp_goal = motivated = 0
## i.e. comparing group-focused reappraisal vs. motivation only
emoreg_data_treat$grp_goal_simp3 <- NA
emoreg_data_treat$grp_goal_simp3[emoreg_data_treat$grp_goal == "motivated"] <- 0
emoreg_data_treat$grp_goal_simp3[emoreg_data_treat$grp_goal == "non_motivated"] <- 1
lm_simp_tr <- lmer(rate ~ grp_goal_simp3*grp_method + size + trial + (1|img) +  (1|groupID/sub_id), emoreg_data_treat)
my_summ_lm(lm_simp_tr)

# Simple effect of motivation -- recode method

## when grp_method = observing = 0
## i.e., comparing motivation only vs. baseline
emoreg_data_treat$grp_method_simp2 <- NA
emoreg_data_treat$grp_method_simp2[emoreg_data_treat$grp_method == "reappraisal"] <- 1
emoreg_data_treat$grp_method_simp2[emoreg_data_treat$grp_method == "observing"] <- 0

lm_simp_tr4 <- lmer(rate ~ grp_goal*grp_method_simp2 + size + trial + (1|img) +  (1|groupID/sub_id),
                    control = lmerControl(optimizer ="Nelder_Mead"), emoreg_data_treat)
my_summ_lm(lm_simp_tr4)
#confint(lm_simp_tr4)



## when grp_method = reapp = 0
## i.e., comparing group-focused reappraisal vs. reappraisal only
emoreg_data_treat$grp_method_simp3 <- NA
emoreg_data_treat$grp_method_simp3[emoreg_data_treat$grp_method == "reappraisal"] <- 0
emoreg_data_treat$grp_method_simp3[emoreg_data_treat$grp_method == "observing"] <- 1

lm_simp_tr3 <- lmer(rate ~ grp_goal*grp_method_simp3 + size + trial + (1|img) +  (1|groupID/sub_id),
                    control = lmerControl(optimizer ="Nelder_Mead"), emoreg_data_treat)
my_summ_lm(lm_simp_tr3)

# diagonal contrast
emm_options(lmer.df = "satterthwaite")
emm <- emmeans(lmm_treat, ~ grp_goal * grp_method, lmerTest.limit = 19356)

## Compare (1,1) vs (0,0)
## i.e. comparing motivation + reappraisal vs. baseline
contr <- contrast(emm, method = list("11 vs 00" = c(-1, 0, 0, 1)))  # Order = (0,0), (0,1), (1,0), (1,1)
summary(contr)
confint(contr)

## Compare (1,0) vs (0,1)
## i.e. comparing motivation only vs. reapp only
contr2 <- contrast(emm, method = list("10 vs 01" = c(0, 1, -1, 0)))  # Order = (0,0), (0,1), (1,0), (1,1)
summary(contr2)
confint(contr2)
```
...
condition_colors <- c('Baseline' = 'black', 'Reappraisal Only' = 'blue',
            'Group-Focused Motivation Only' = 'red', 'Group-Focused Motivation + Reappraisal' = 'purple')

# bar plot
treat_summary <- data.frame(
  Condition = c('Baseline', 'Reappraisal Only',
                'Group-Focused Motivation Only', 'Group-Focused Motivation + Reappraisal'),
  Score = c(6.37, 4.57, 4.80, 3.91),
  SD = c(0.232, 0.244, 0.240, 0.252)  # summary data from emmeans est.
)

treat_summary$Condition <- factor(treat_summary$Condition, levels = c('Baseline', 'Group-Focused Motivation Only',
                'Reappraisal Only', 'Group-Focused Motivation + Reappraisal'))
...
ggplot(treat_summary, aes(x = Condition, y = Score, color = Condition)) +
  ...
  theme(legend.position = "none",  # Remove legend
        ...
        axis.text.x = element_text(size = rel(1.0)),  # Increase size of x-axis text
...
ggsave("../Figures/Figure 2-treated-only.png", width = 8, height =6, dpi=300)
...
# Simple effect of method -- recode motivation
## when grp_goal = motivated = 0
## i.e. comparing motivation only vs. motivation+reappraisal
...
## i.e. comparing reappraisal only vs. reappraisal + motivation
...
# diagonal contrast
emm_options(lmer.df = "satterthwaite")
emm <- emmeans(lmm_cont, ~ grp_goal * grp_method, lmerTest.limit = 19356)
...
summary_control <- data.frame(
  group_cond_new = c('Baseline', 'Mere Reappraisal',
                'Mere Group-Focused Motivation', 'Reappraisal+Group-Focused Motivation'),
...
summary_control$group_cond_new <- factor(summary_control$group_cond_new, 
                                         levels = c('Baseline', 'Mere Reappraisal', 'Mere Group-Focused Motivation',
                 'Reappraisal+Group-Focused Motivation'))


colors <- c("Baseline" = "black", "Mere Reappraisal" = "blue", 
            "Mere Group-Focused Motivation" = "red", "Reappraisal+Group-Focused Motivation" = "purple")
...
plot_cont_sum <- ggplot(summary_control, aes(x = group_cond_new, y = mean_rate, color = group_cond_new)) +
  ...
  theme(legend.position = "none",  # Remove legend
        ...
        panel.grid.minor = element_blank(),  # Remove minor grid lines
        axis.line = element_line(colour = "black"),
        axis.text.x = element_text(size = rel(1.2), angle = 15, hjust = 1),  # Increase size of x-axis text
        axis.text.y = element_text(size = rel(1.5)),  # Increase size of y-axis text
        axis.title.x = element_text(size = rel(1.5)),  # Increase size of x-axis title
        axis.title.y = element_text(size = rel(1.5))) +  # Add axis lines
  labs(x = "Treatment Condition", y = "Non-Treated Participants' Negative Emotions")  

plot_cont_sum
ggsave("../Figures/Figure 2-non-treated-only.png", width = 8, height =6, dpi=300)
