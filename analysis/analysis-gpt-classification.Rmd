...
my_summ_lm <- function(x, ...) {
    jtools::summ(x, scale=F, model.info = T , confint = T, digits = 3, ...)
}
...
my_summ_lm(m2_treat_main)



# Increase in reappraisal propensity in the mere motivation condition
...
## Change in reaprpaisal over time in other conditions
lm_control_only <- glmer(gpt_label_bi ~  trial +
              (1|groupID/sub_id) + (1 | img),
            data = emoreg_dat %>% filter(group_cond == "control"), 
            family = binomial, control = glmerControl(optimizer = "bobyqa"))
my_summ_lm(lm_control_only)


lm_reappraisal_only <- glmer(gpt_label_bi ~  trial +
              (1|groupID/sub_id) + (1 | img),
            data = emoreg_dat %>% filter(subgrp == "treatment" & group_cond == "reappraisal"), 
            family = binomial, control = glmerControl(optimizer = "bobyqa"))
my_summ_lm(lm_reappraisal_only)

lm_reappraisal_motivation <- glmer(gpt_label_bi ~  trial +
              (1|groupID/sub_id) + (1 | img),
            data = emoreg_dat %>% filter(subgrp == "treatment" & group_cond == "reappraisal & motivation"), 
            family = binomial, control = glmerControl(optimizer = "bobyqa"))
my_summ_lm(lm_reappraisal_motivation)
```

```{r simple_treat}
# Simple effects of the treatment subgroup


## model 1 - simple effect of strategy under no motivation
## code no motivation = 0
emoreg_dat$grp_goal3 <- NA
emoreg_dat$grp_goal3[emoreg_dat$grp_goal == "motivated"] <- 1
emoreg_dat$grp_goal3[emoreg_dat$grp_goal == "non_motivated"] <- 0

glm_simp1 <- glmer(gpt_label_bi ~ grp_method*grp_goal3 + trial +
                   (1|groupID/sub_id) + (1 | img),
            data = emoreg_dat %>% filter(subgrp == "treatment" | (subgrp == "control" & group_cond == "control")), 
            family = binomial, control = glmerControl(optimizer = "bobyqa"))

my_summ_lm(glm_simp1)

## model 2 - simple effect of strategy under group motivation
## code group motivation = 0
emoreg_dat$grp_goal4 <- NA
emoreg_dat$grp_goal4[emoreg_dat$grp_goal == "motivated"] <- 0
emoreg_dat$grp_goal4[emoreg_dat$grp_goal == "non_motivated"] <- 1


glm_simp2 <- glmer(gpt_label_bi ~ grp_method*grp_goal4 + trial +
              (1|groupID/sub_id) + (1 | img),
            data = emoreg_dat %>% filter(subgrp == "treatment" | (subgrp == "control" & group_cond == "control")), 
            family = binomial, control = glmerControl(optimizer = "bobyqa"))

my_summ_lm(glm_simp2)



## model 3 - simple effect of motivation under observing
## code observing strategy = 0
emoreg_dat$grp_method3 <- NA
emoreg_dat$grp_method3[emoreg_dat$grp_method == "reappraisal"] <- 1
emoreg_dat$grp_method3[emoreg_dat$grp_method == "observing"] <- -0


glm_simp3 <- glmer(gpt_label_bi ~ grp_method3*grp_goal + trial +
              (1|groupID/sub_id) + (1 | img),
            data = emoreg_dat %>% filter(subgrp == "treatment" | (subgrp == "control" & group_cond == "control")), 
            family = binomial, control = glmerControl(optimizer = "bobyqa"))

my_summ_lm(glm_simp3)

## model 4 - simple effect of motivation under reappraisal
## code reapp strategy = 0
emoreg_dat$grp_method4 <- NA
emoreg_dat$grp_method4[emoreg_dat$grp_method == "reappraisal"] <- 0
emoreg_dat$grp_method4[emoreg_dat$grp_method == "observing"] <- -1


glm_simp4 <- glmer(gpt_label_bi ~ grp_method3*grp_goal + trial +
              (1|groupID/sub_id) + (1 | img),
            data = emoreg_dat %>% filter(subgrp == "treatment" | (subgrp == "control" & group_cond == "control")), 
            family = binomial, control = glmerControl(optimizer = "bobyqa"))

my_summ_lm(glm_simp4)



# Diagonal contrast
emm_options(lmer.df = "satterthwaite")
emm <- emmeans(m2_treat, ~ grp_goal * grp_method, lmerTest.limit = 19356)
...
## Compare (0,1) vs (1,0)
## i.e. comparing motivation only vs. reappraisal only
contr2 <- contrast(emm, method = list("11 vs 00" = c(0, -1, 1, 0)))
summary(contr2)
confint(contr2)
```
...
glm_control_simp1 <- glmer(gpt_label_bi ~ grp_method*grp_goal3 + trial +
                   (1|groupID/sub_id) + (1 | img),
            data = emoreg_dat %>% filter(subgrp == "control"), 
            family = binomial, control = glmerControl(optimizer = "bobyqa"))

my_summ_lm(glm_control_simp1)

## model 2 - simple effect of strategy under group motivation
## code group motivation = 0

glm_control_simp2 <- glmer(gpt_label_bi ~ grp_method*grp_goal4 + trial +
              (1|groupID/sub_id) + (1 | img),
            data = emoreg_dat %>% filter(subgrp == "control"), 
            family = binomial, control = glmerControl(optimizer = "bobyqa"))

my_summ_lm(glm_control_simp2)
...
glm_control_simp3 <- glmer(gpt_label_bi ~ grp_method3*grp_goal + trial +
              (1|groupID/sub_id) + (1 | img),
            data = emoreg_dat %>% filter(subgrp == "control"), 
            family = binomial, control = glmerControl(optimizer = "bobyqa"))

my_summ_lm(glm_control_simp3)

## model 4 - simple effect of motivation under reappraisal
## code reapp strategy = 0

glm_control_simp4 <- glmer(gpt_label_bi ~ grp_method4*grp_goal + trial +
              (1|groupID/sub_id) + (1 | img),
            data = emoreg_dat %>% filter(subgrp == "control"), 
            family = binomial, control = glmerControl(optimizer = "bobyqa"))

my_summ_lm(glm_control_simp4)

## diagonal contrast
emm_options(lmer.df = "satterthwaite")
emm <- emmeans(m2_control, ~ grp_goal * grp_method, lmerTest.limit = 19356)
...
## Compare (0,1) vs (1,0)
## i.e. comparing motivation only vs. reappraisal only
contr <- contrast(emm, method = list("10 vs 01" = c(0, -1, 1, 0)))  
summary(contr)
confint(contr)

```

## Only looking at the first few trials
```{r}
